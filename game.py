...
class Game:
    ...
    def _update(self) -> None:
        ...
        items = game.get_actors()
        for item in items:
            if isinstance(item, actor.Is):
                is_block = self.get_actor(item.x, item.y)
                right_is = self.get_actor(is_block.x + 1, item.y)
                left_is = self.get_actor(is_block.x - 1, item.y)
                up_is = self.get_actor(item.x, is_block.y - 1)
                down_is = self.get_actor(item.x, is_block.y + 1)

                rules = actor.Is.update(item, up_is, down_is, left_is, right_is)
                if rules not in lst_of_rules:
                    lst_of_rules.append(rules)

        #  P.S - Sorry about the design, it was late.
        #  P.S.S - Have mercy on grading this, please.

        #  Set Meepo
        for item in lst_of_rules:
            #  Is You Conditions
            if "Meepo isYou" in item:
                for item in self._actors:
                    if isinstance(item, actor.Meepo):
                        self.player = item
                        meepo_alive = True
                    ...
                    if meepo_alive:
                        #  Push Conditions
                        for item in lst_of_rules:
                            if "Rock isPush" in item:
                                for thing in self._actors:
                                    if isinstance(thing, actor.Rock):
                                        thing._is_push = True
                                        thing._is_stop = False

                            if "Wall isPush" in item:
                                for thing in self._actors:
                                    if isinstance(thing, actor.Wall):
                                        thing._is_push = True
                                        thing._is_stop = False

                            if "Flag isPush" in item:
                                for thing in self._actors:
                                    if isinstance(thing, actor.Flag):
                                        thing._is_push = True
                                        thing._is_stop = False

                            #  Win Conditions
                            if "Rock isVictory" in item:
                                self.win()

                            if "Wall isVictory" in item:
                                self.win()

                            if "Flag isVictory" in item:
                                self.win()

                            #  Lose Conditions
                            if "Wall isLose" in item:
                                self.lose()

                            if "Wall isLose" in item:
                                self.lose()

                            if "Flag isLose" in item:
                                self.lose()
